...
class STMControlNode(Node):
    def __init__(self):
        ...
        self.master_state_topic = f"/group_{self.master_group_id}/stm_state"  # Master stm_state topic (/group_id/stm_state)  # Master stm_state topic (/group_id/stm_state)
        self.slave_state_topic = f"/group_{self.slave_group_id}/stm_state"        
        self.slave_control_topic = f"/group_{self.slave_group_id}/stm_control" # Slave stm_control topic (/group_id/stm_control)  # Slave stm_state topic (/group_id/stm_state)
        ...
        self.slave_stm_state_subscriber = self.create_subscription(
            ...
            self.slave_stm_state_callback,
            10
        )   # Subscriber for the slave node state

        self.master_stm_state_subscriber
        self.slave_stm_state_subscriber
        
        self.get_logger().info(f"Subscribed to {self.master_state_topic}")
        self.get_logger().info(f"Subscribed to {self.slave_control_topic}")
    ...
    def master_stm_state_callback(self, msg):
        ...
        self.master_velocity = msg.motor_velocity 
    ...
    def slave_stm_state_callback(self, msg):
        ...
        """
        # TODO: Update master_position with value from msg
        self.slave_position = msg.motor_encoder
        self.slave_velocity = msg.motor_velocity 
    ...
    def service_control_type_callback(self, request, response):
        ...
        self.get_logger().info(f"Control gain set to: kp={self.kv}")
    ...
    def timer_callback(self):
        ...
        if self.control_type >=0 and self.control_type <=2:
            ...
            error_pos = self.slave_position - self.master_position
            # TODO: Calculate the velocity error between the master and slave encoder velocities
            error_vel = self.master_velocity - self.slave_velocity
            # TODO: Calculate the control command using the gain constants
            control_command = self.kp*error_pos + self.kv*error_vel
            ...
            control_msg.pwm = control_command
        ...
        self.get_logger().info(f"errror pos command: {error_pos}")
        self.get_logger().info(f"error velocity command: {error_vel}")
